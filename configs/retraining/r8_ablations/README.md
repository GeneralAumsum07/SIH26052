...
run; the configs are generated by the same script as the full configs, so the untouched fields cannot drift.

Arms whose value equals the r8_fe_mini default are **not duplicated**: the baseline for ablations 2, 3, 3b, 4 and 6 is
`ab1_fe_mini_s0` (and `_s1` where two seeds are listed): inputs `pr`, reference absent 15 %, tail share 25 %,
...
| 3b | Low-ILD tail share of the M8 scenes | `ab3b_tail00`, `ab3b_tail10` | `data.mix.v2.tail_share` 0 / 0.10 | `ab1_fe_mini_s0` (0.25) |
...
TBD (Rachit, plan 11.9 Q7): the selection metric. The pilots use `val.select: composite`; comparing arms by the
