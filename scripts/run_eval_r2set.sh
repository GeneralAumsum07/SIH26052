...
for s in raw nlms_only gtcrn_pretrained; do
  [ -f results_r2/$s.csv ] || uv run python -m vaani.eval --system $s --split test --eval-root data/eval_r2 --out results_r2/$s.csv --asr > results_r2/eval_$s.log 2>&1
done
for r in gtcrn_finetuned vaani_no_controller vaani_full vaani_full_sp; do for x in "" _r2; do r2=$r$x
  [ -f results_r2/$r2.csv ] || uv run python -m vaani.eval --system ckpt:runs/$r2/best.pt --split test --eval-root data/eval_r2 --out results_r2/$r2.csv --asr > results_r2/eval_$r2.log 2>&1
done; done
[ -f results_r2/asr/clean.csv ] || uv run python scripts/asr_clean_reference.py --eval-root data/eval_r2 --out results_r2/asr/clean.csv > results_r2/asr/clean.log 2>&1
