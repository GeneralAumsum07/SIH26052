...
def make_system(spec):
    ...
    if spec == "r7" or spec.startswith("stream:"):
        ...
        def run(p, r, trace=False):
            return physical.run_engine(np.stack([p, r]), onnx, config, 1, trace=trace)
        run.stream = True
        return run
...
def run_part2(a, wav=WEB_WAV):
    ...
    fn = system(a.system)
    key = a.validity_key
    res = {"wav": str(wav), "seconds": len(L) / SR, "whisper_available": have_whisper(), "runs": {}}
    in_words = {}
    for name, (p, r) in runs.items():
        y, diag = fn(p, r, trace=True) if fn.stream else fn(p, r)
        ...
        for c in CONS:
            ...
            if c == "H8":
                ...
                if h:
                    ...
                    st["ps_pesq"] = float(np.nanmean([r["pesq_out"] for r in h]))
...
def write_md(res, path):
    L = [f"# G4 field acceptance: {res['name']} (`{res['system']}`)", "",
         f"Generated by `{res['command']}`. Part 1 verdict: **{res['part1_verdict']}**; Part 2 verdict: "
         f"**{res['part2_verdict']}**. Definitions and criteria: scripts/field_accept.py docstring (plan 11.2 G4).", "",
         ...
         "| bed/cons | n | loss mean | loss p95 | lost run max s | STOI in -> out | SNR in -> out | dSNR | PESQ out | "
         "verdict | failed criteria |", "|---|---|---|---|---|---|---|---|---|---|---|"]
    for k, st in res["part1"].items():
        if k.endswith("gtcrn_pretrained"):
            L.append(f"| {k} | {st['n']} | {_f(st['loss_mean'])} | | | | | {_f(st['dsnr_mean'], 2)} | | ref | |")
            continue
        bad = ", ".join(c for c, v in st["criteria"].items() if v != "PASS") or "-"
        L.append(f"| {k} | {st['n']} | {_f(st['loss_mean'])} | {_f(st['loss_p95'])} | {_f(st['lost_run_max_s'], 2)} | "
                 f"{_f(st['stoi_in'])} -> {_f(st['stoi_out'])} | {_f(st['snr_in'], 1)} -> {_f(st['snr_out'], 1)} | "
                 f"{_f(st['dsnr_mean'], 2)} | {_f(st['pesq_out'], 2)} | {st['verdict']} | {bad} |")
    ...
    if h8:
        L += ["", "H8 at +5 dB (PS targets SNR_out > 15, STOI > 0.85, PESQ > 2.5): " + "; ".join(
            f"{k}: {_f(st['ps_snr_out'], 1)} / {_f(st['ps_stoi'])} / {_f(st['ps_pesq'], 2)}" for k, st in h8)]
    if res.get("part2"):
        p2 = res["part2"]
        L += ["", f"## Part 2: reference-free, `{p2['wav']}` ({p2['seconds']:.1f} s)", "",
              ...
              "| run | longest >30 dB s | atten>20dB frac | proxy survival | VAD speech s (in) | word survival | "
              "validity latency s |", "|---|---|---|---|---|---|---|"]
        for n, r in p2["runs"].items():
            L.append(f"| {n} | {_f(r['longest_atten30_s'], 2)} | {_f(r['atten20_frac'])} | {_f(r['proxy_survival'])} | "
